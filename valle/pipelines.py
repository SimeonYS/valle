...
class VallePipeline:
    ...
    # Database setup
    conn = sqlite3.connect('valle.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `valle`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM valle WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO valle (date, title, link, content)"
